...
class Gauss:    
    ...
    def FuncApproxF(self,Xdata,length,centers,beta,R,D,stim,tau):
        ...
        PhiMatVolt = self.CreatePhiMatVoltage(XdataT,C,length,NcLength,R,stim,D)
        ...
        def newfunc(x,stim):
            ...
            #    f += W[c]*np.exp(-(np.linalg.norm(x-C[c])**2)*R)
    ...
    def PredictIntoTheFuture(self,F,PreLength,stim,PData):
        ...
        def makePre(D,PData,tau):           
            ...
            Prediction[tau*(D-1)+1] = Prediction[tau*(D-1)]+F(Input,stim[0:2])
